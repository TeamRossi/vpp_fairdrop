echo -e "\n\n\n$VPP_ROOT\n\n\n"
sudo $SFLAG $BINS/vppctl -p vpp2 set ip arp $NAMELC1P1 $IPV4_VPP_IPROUTE $MACLC0P1 # 137.194.208.1 is just a random ip address. It can be anything. The same ip address should be used in ip route command. We are just forwarding all packets to one interface.
...
sudo $SFLAG $BINS/vppctl -p vpp2 set int ip addr $NAMELC1P1 $IPV4_VPP_INTERFACE1 #192.168.2.2/32
sudo $SFLAG $BINS/vppctl -p vpp2 set int ip addr $NAMELC1P0 $IPV4_VPP_INTERFACE0 #192.168.2.3/32
