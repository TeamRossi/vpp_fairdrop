...
pktgen.start(0);
--pktgen.delay(20000);
os.execute("$SCRIPTS/runs.sh 'clear run' ");
pktgen.delay(SLEEP*SECONDS);
os.execute("$SCRIPTS/runs.sh 'show run' > /tmp/showrun.dat ");
os.execute("$SCRIPTS/runs.sh 'show interface' > /tmp/showint.dat ");
